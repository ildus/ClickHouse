        ...
        for (size_t i = 0; i < row_count; i++)
        {
            ...
            for (auto & arg : args)
            {
                ...
                for (size_t j = 0; j < len; j++)
                {
                    ...
                    {
                        key = assert_cast<const ColumnString *>(arg.key_column.get())->getDataAt(offset + j).toString();
                    }
                    else
                    {
                    ...
                    ValType value = assert_cast<const ColumnVector<ValType> *>(arg.val_column.get())->getData()[offset + j];
        ...
        if (tup_type)
        {
            ...
            for (const auto & col : arguments)
            {
                ...
                if (is_const)
                {
                    ...
                    const auto & unwrapped_key_column = assert_cast<const ColumnConst *>(key_column.get())->getDataColumnPtr();
                    const auto & unwrapped_val_column = assert_cast<const ColumnConst *>(val_column.get())->getDataColumnPtr();
                    args.push_back({unwrapped_key_column, unwrapped_val_column, key_offsets, val_offsets, is_const});
                }
                else
